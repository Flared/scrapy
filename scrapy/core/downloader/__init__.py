...
def _get_concurrency_delay(
    concurrency: int, request: Request, spider: Spider, settings: BaseSettings
) -> Tuple[int, float]:
    ...
    delay = request.meta.get("slot_download_delay", delay)
    concurrency = request.meta.get("slot_concurrency", concurrency)
...
class Downloader:
    ...
    def _get_slot(self, request: Request, spider: Spider) -> Tuple[str, Slot]:
        ...
        if key not in self.slots:
            ...
            conc, delay = _get_concurrency_delay(conc, request, spider, self.settings)
            conc, delay = (
    ...
    def _download(self, slot: Slot, request: Request, spider: Spider) -> Deferred:
        ...
        dfd = self.signals.send_deferred(
            signal=signals.request_downloading, request=request, spider=spider
        )
